...
*[![gnome-weather](https://snapcraft.io/gnome-weather/badge.svg)](https://snapcraft.io/gnome-weather)
[![gnome-weather](https://snapcraft.io/gnome-weather/trending.svg?name=0)](https://snapcraft.io/gnome-weather)*
...
git remote add ubuntu https://github.com/ubuntu/gnome-weather.git
git fetch --all
```
...
5. Once the PR has been merged into the stable branch, then on the next git mirror sync (every 4 hours), launchpad will trigger [a build of the snap that gets published](https://launchpad.net/~desktop-snappers/+snap/gnome-weather) to the [snap store](https://snapcraft.io/gnome-weather) into the *candidate* channel. After sufficient testing of the snap from the candidate channel, then the reviewer (a Collaborator of the snap in the store) will promote the snap to the stable branch in the snap store.
